...
def prepare(args: argparse.Namespace, config: dict, profile: dict, run_root: Path) -> dict:
    ...
    model_path = run_root / "model.mtace.pt"
    from mtace.deployment import export_lammps_model

    # The MTACE exporter generates and validates its own trace example on a
    # different atom and edge count than it traced with, checks the coordinate
    # gradient and the strain virial, and forces the portable differentiable
    # backend.  It therefore needs only the element order.
    export_lammps_model(
        checkpoint_path=checkpoint,
        output_path=model_path,
        elements=list(config["system"]["type_map"]),
    )
