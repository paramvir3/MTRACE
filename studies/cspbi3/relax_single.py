#!/usr/bin/env python3
"""Relax one periodic structure with a MTACE checkpoint.

Ported from the Transformers-ACE reference script; only the calculator and the
repository-root detection differ.
"""
...
REPO_ROOT = next(
    (parent for parent in SCRIPT_PATH.parents if (parent / "mtace" / "__init__.py").is_file()),
    None,
)
if REPO_ROOT is None:
    raise RuntimeError(
        "Could not locate the MTACE repository. Keep this script inside the "
        "repository tree, or install the mtace package first."
    )
...
def main():
    ...
    calculator = MambaACECalculator(model_path, device=args.device)
