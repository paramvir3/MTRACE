#!/usr/bin/env python3
"""Evaluate relative energies of periodic CsPbI3 polymorphs with MTACE.

Ported from the Transformers-ACE reference script of the same name; the
thermodynamics, the phase set, the reference conventions and the output schema
are unchanged so the two runs are directly comparable.  Only the calculator
differs.

Relative energies are reported per formula unit.  Every structure here is
stoichiometric CsPbI3, so the constant per-atom reference energy stored in the
checkpoint cancels exactly in every difference and cannot influence the ordering.
"""
...
from mtace import MambaACECalculator
...
def parse_args():
    ...
    parser.add_argument("--model", required=True, type=Path, help="MTACE .pt checkpoint")
...
def main():
    ...
    calculator = MambaACECalculator(args.model, device=args.device)
    rows = []
